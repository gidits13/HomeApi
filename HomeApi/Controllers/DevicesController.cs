...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        [Route("delete")]
        [HttpPost]
        public async Task<IActionResult> DeleteDiveice(DeleteDeviceRequest request)
        {
            var device = await _devices.GetDeviceById(request.Guid);
            if (device is null)
                return StatusCode(400,$"Устройство не найдено({request.Guid})");
            await _devices.DeleteDevice(device);
            return StatusCode(200, $"Устройство успешно удалено({device.Name}, {device.SerialNumber})");
